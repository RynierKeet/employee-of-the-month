// src/components/AdjudicationPanel.tsx
...
/* -----------------------------------------------------
   Simple Accordion used in Round History
----------------------------------------------------- */
function Accordion({
  title,
  children,
...
export default function AdjudicationPanel() {
  ...
  useEffect(() => {
    ...
    // polling to keep adjudicators in sync
  ...
  async function castRoundVote(candidateId: number) {
    ...
    try {
      ...
      if (!res.ok) {
        ...
        setMessage(data?.error || `Failed to cast vote (${res.status}).`);
        return;
      }
      ...
      // If not all adjudicators have voted yet, just refresh
  ...
  return (
    <div className="bg-white shadow-card border border-slate-200 rounded-card p-8 space-y-8">
            ...
            <div className="p-3 border border-crgGold bg-[#fff9e8] rounded-card">
              ...
              <div className="mt-2">
                <button
                  onClick={startRound}
                  disabled={loading}
                  className="px-3 py-1 rounded-card font-medium text-white bg-brandnavy hover:bg-slate-800 hover:text-crgGold transition disabled:opacity-50"
                >
                  Start Next Round
                </button>
              </div>
              ...
              >
                ...
                {!roundWinner && (
                  <button
                    ...
                    disabled={loading || isVoting || voteSaved}
      ...
      {/* Candidate grid (initial state, before adjudication round) */}
      {!currentRound && !finalWinner && (
        <div className="grid grid-cols-1 sm:grid-cols-2 xl:grid-cols-3 gap-6">
          {candidates.map((c) => {
